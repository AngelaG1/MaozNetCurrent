...
namespace Network
{
    ...
    public class Network
    {
        ...
        protected MatMultForm _Form2 = new MatMultForm();
        ...
        public void LoadStructEquiv(double maxik, int year, string m) //5-14
        {
            // may not need
            /*
        ...
        //Alvin Main
        public List<List<Matrix>> ABMShocksNetworkFormation(int nodecount, int network, string filename, int runno, bool homophily,
                                                            bool enemy, bool cultism, bool democracy, ref ABMProgressForm apform, bool usrInput, string usrFile)
        {
            Matrix modeldata = new Matrix(nodecount * nodecount, 32); //editted Alvin

           
                //new variable neighborofenmy ModelData[29] randomly assinging nodes
                for (int i = 0; i < nodecount * nodecount; i++)
                {
                    //generating random neighbors
                    double rng = RNGen.NextDouble();

                    if (modeldata[i, 7] == 1)
                        modeldata[i, 29] = rng > .75 ? 1 : 0;
                    else
                        modeldata[i, 29] = rng > .75 ? 1 : 0;
                }
        ...
        public double calculateHomophilyUtility(int row, int col, int nodecount, int netnodes, ref Matrix modeldata) //editted by Alvin
        {
            List<double> firstorderutility = new List<double>();
            double utility, cost, netutility;

            //di_dj
            Matrix dijMatrix = new Matrix(nodecount, nodecount);

            Matrix test = new Matrix(nodecount, nodecount);

            double[] di = new double[nodecount];
            double[] dj = new double[nodecount];

            for (int i = 0; i < nodecount * nodecount; i++)
            {
                int d_row = (int)modeldata[i, 2];
                int d_col = (int)modeldata[i, 3];
                double edge = modeldata[i, 4];

                dijMatrix[--d_row, --d_col] = edge;
            ...
            //assigning di dj to respect rows
            for (int i = 0; i < nodecount * nodecount; i++)
            {
                int d_row = (int)modeldata[i, 2];
                int d_col = (int)modeldata[i, 3];

                modeldata[i, 30] = di[--d_row];
                modeldata[i, 31] = dj[--d_col];
            }
                //demcol = 10
                //demcrow = 7
                //enemy = 11, 8
            ...
            for (int i = 0; i < nodecount; ++i)
            {
                //labeing variables for easier calculations

                double demcol = modeldata[row * nodecount + i, 10];
                double comEnmy = modeldata[row * nodecount + i, 11] * modeldata[row * nodecount + i, 8];
                double cultism = modeldata[row * nodecount + i, 12]  * modeldata[row * nodecount + i, 9];
                double NeighEnemy = modeldata[row * nodecount + i, 29] ;

                if (modeldata[row, 7] == 1)
                {

                   // cost = (.5 * modeldata[row * nodecount + i, 10]) + (.3 * modeldata[row * nodecount + i, 11]) + (.2 * modeldata[row * nodecount + i, 9] * modeldata[row * nodecount + i, 12]);
                    utility = (.4 * demcol) + (.25 * comEnmy) + (.15 * cultism) + (.2 * NeighEnemy);

                    if (row == col)
                        utility = 0;
                    //cost = netnodes > 0 ? utility / netnodes : 0;

                    cost = utility * (modeldata[row * nodecount + i, 31] - modeldata[row * nodecount + i, 4]) / (nodecount - 1);
                    netutility = utility - cost > 0 ? utility - cost : 0;
                ...
                else
                {
                     ...
                     //cost = (.5 * modeldata[row * nodecount + i, 11]) + (.3 * modeldata[row * nodecount + i, 12]) + (.2 * modeldata[row * nodecount + i, 10]);
                    utility = (.15 * demcol) + (.4 * comEnmy) + (.25 * cultism) + (.2 * NeighEnemy);
            ...
            //modeldata[row * nodecount + col, 28] = Math.Pow(dijMatrix[--ujk_C, ujk_R - 1], 2);
    ...
    public class NetworkIO : Network
    {
        ...
        public void LoadMultiplicationMatrix(string filename, bool dyadic, int year, string targetMatrix, string sourceMatrix) //Alvin here
        {
            ...
            //mTable[targetMatrix] = mTable[sourceMatrix];
            //Matrix test = mTable[sourceMatrix];
    ...
    public class NetworkGUI : NetworkIO
    {
        ...
        public void LoadMultiplexNull(DataGridView data, int year, string[] fnames, string curFile, string LoadFrom, bool order)
        {
            int numNodes = mTable["Data"].Cols;
            int numCells = numNodes * numNodes;

            //loadFile Procedure
            Matrix plus = new Matrix(numNodes, numNodes);
            Matrix neg = new Matrix(numNodes, numNodes);

            double plusDensity = 0;
            double negDensity = 0;
            bool zeroDiag = true;

            //loading procedure: density and tags from base matrix
            List<string> tags = new List<string>();

            if (fnames == null)
            {
                if (LoadFrom == "Matrix") //matrix load
                {

                }

                else //assumption dyadic File
                {
            ...
            else
            {
                if (LoadFrom == "Dyadic") //multi-variable
                {
                    ...
                    List<string> networkLines = new List<string>();
                    sr.ReadLine(); // clear header

                    while (!sr.EndOfStream) //loading year
                    {
                    ...
                    for (int i = 0; i < numNodes; i++)
                    {
                        ...
                        {
                            var vals = networkLines[l++].Split(',');

                            for (int k = 3; k < vals.Length; k++)
                            {
                                double d = double.Parse(vals[k]);
                                plus[i, j] += d > 0 ? d : 0;
                                neg[i, j] += d < 0 ? d * -1 : 0;
                                plusDensity += d > 0 ? d : 0;
                                negDensity += d < 0 ? d * -1 : 0;
             ...
             // End of load File procedure-------------------------------------------------------

            Matrix rand = calcDyadicNull(plus, neg, plusDensity, negDensity, zeroDiag);

            Matrix r = plus + neg;
            Matrix ro = elementMatrixMult(plus, neg);

            Matrix bal = new Matrix(numNodes, numNodes);
            Matrix imbal = new Matrix(numNodes, numNodes);
            Matrix b2 = new Matrix(numNodes, numNodes);
            Matrix i2 = new Matrix(numNodes, numNodes);
            ...
            for (int i = 0; i < numNodes; i++)
            {
                ...
                {
                    //first order
                    bal[i, j] = r[i, j] > 1 ? ((plus[i, j] * (plus[i, j] - 1)) / (r[i, j] * (r[i, j] - 1))) : -99;
                    imbal[i, j] = r[i, j] > 1 ? ((ro[i, j] * 2) / (r[i, j] * (r[i, j] - 1))) : -99;

                    //second order
                    b2[i, j] = r2[i, j] > 1 ? ((rp2[i, j] * (rp2[i, j] - 1)) / (r2[i, j] * (r2[i, j] - 1))) : -99;
                    i2[i, j] = r2[i, j] > 1 ? ((rpn2[i, j] * 2) / (r2[i, j] * (r2[i, j] - 1))) : -99;
                }
            }
            ...
            //outputing results_______________________________________________________________________
            data.Columns.Clear();
            Matrix output = null;
            ...
            if (order)
            {
                output = mTable.AddMatrix("Multiplex", numCells, 15);
            }

            else
            {
                output = mTable.AddMatrix("Multiplex", numCells, 25);
                output.ColLabels[15] = "b^2";
                output.ColLabels[16] = "mean(b^2)";
                output.ColLabels[17] = "sd(b)";
                output.ColLabels[18] = "lci(b)";
                output.ColLabels[19] = "uci(b)";
                output.ColLabels[20] = "i^2";
                output.ColLabels[21] = "mean( i^2)";
                output.ColLabels[22] = "sd( i^2)";
                output.ColLabels[23] = "lci( i^2)";
            ...
            output.ColLabels[4] = "X-";
            output.ColLabels[5] = "b";
            output.ColLabels[6] = "mean(b)";
            output.ColLabels[7] = "sd(b)";
            output.ColLabels[8] = "lci(b)";
            output.ColLabels[9] = "uci(b)";
            output.ColLabels[10] = "imb";
            output.ColLabels[11] = "mean(imb)";
            output.ColLabels[12] = "sd(imb)";
            output.ColLabels[13] = "lci(b)";
            output.ColLabels[14] = "uci(b)";
            ...
            for (int i = 0; i < numCells; i++)
            {
                ...
                //imbal
                output[i, 10] = imbal[a, b];
                output[i, 11] = rand[i, 4];
                output[i, 12] = rand[i, 5];
                output[i, 13] = rand[i, 6];
                output[i, 14] = rand[i, 7];

                if (!order)
                {
                    output[i, 15] = b2[a, b];
                    output[i, 16] = rand[i, 8];
                    output[i, 17] = rand[i, 9];
                    output[i, 18] = rand[i, 10];
                    output[i, 19] = rand[i, 11];

                    //i^2
                    output[i, 20] = i2[a, b];
                    output[i, 21] = rand[i, 12];
                    output[i, 22] = rand[i, 13];
                    output[i, 23] = rand[i, 14];
                    output[i, 24] = rand[i, 15];
                }
            ...
            for (int i = 0; i < numNodes; i++)
            {
                for (int j = 0; j < numNodes; j++)
                {
                    output[dummy, 1] = double.Parse(tags[i]);
                    output[dummy++, 2] = double.Parse(tags[j]);
                }
            }
        ...
        } //end of dydaic null model

        //refb
        public Matrix calcDyadicNull(Matrix p, Matrix n, double pT, double nT, bool hasDiag) //p = plus, n = neg, pT = denstiy of plus, nT = denisty of neg
        {
            int nodes = p.Rows;
            Matrix meanb = new Matrix(nodes, nodes);
            Matrix sdb = new Matrix(nodes, nodes);
            Matrix meani = new Matrix(nodes, nodes);
            Matrix sdi = new Matrix(nodes, nodes);
            Matrix meanb2 = new Matrix(nodes, nodes);
            Matrix sdb2 = new Matrix(nodes, nodes);
            Matrix meani2 = new Matrix(nodes, nodes);
            Matrix sdi2 = new Matrix(nodes, nodes);


            Matrix final = new Matrix(nodes * nodes, 16);


            for (int k = 0; k < 100; k++)
            {
                Matrix plus = calcNullMatrix(hasDiag, pT, p);
                Matrix neg = calcNullMatrix(hasDiag, nT, n);

                Matrix r = plus + neg;
                Matrix ro = elementMatrixMult(plus, neg);

                Matrix bal = new Matrix(nodes, nodes);
                Matrix imbal = new Matrix(nodes, nodes);
                Matrix b2 = new Matrix(nodes, nodes);
                Matrix i2 = new Matrix(nodes, nodes);
                ...
                for (int i = 0; i < nodes; i++)
                {
                    ...
                    {
                        //first order
                        bal[i, j] = r[i, j] > 1 ? ((plus[i, j] * (plus[i, j] - 1)) / (r[i, j] * (r[i, j] - 1))) : -1;
                        imbal[i, j] = r[i, j] > 1 ? ((ro[i, j] * 2) / (r[i, j] * (r[i, j] - 1))) : -1;

                        //second order
                        b2[i, j] = r2[i, j] > 1 ? ((rp2[i, j] * (rp2[i, j] - 1)) / (r2[i, j] * (r2[i, j] - 1))) : -1;
                        i2[i, j] = r2[i, j] > 1 ? ((rpn2[i, j] * 2) / (r2[i, j] * (r2[i, j] - 1))) : -1;
                    }
                }
                ...
                for (int a = 0; a < nodes; a++)
                {
                    for (int b = 0; b < nodes; b++)
                    {
                        ...
                        //x
                        double bal1 = bal[a, b] >= 0 ? bal[a, b] : 0;
                        double bal2 = b2[a, b] >= 0 ? b2[a, b] : 0;
                        double imb1 = imbal[a, b] >= 0 ? imbal[a, b] : 0;
                        double imb2 = i2[a, b] >= 0 ? i2[a, b] : 0;


                        //oldmean
                        double oldbal1 = meanb[a, b];
                        double oldbal2 = meanb2[a, b];
                        double oldimb1 = meani[a, b];
                        double oldimb2 = meani2[a, b];

                        //mean
                        meanb[a, b] += (bal1 - meanb[a,b]) / c;
                        meanb2[a, b] += (bal2 - meanb2[a, b]) / c;
                        meani[a, b] += (imb1 - meani[a, b]) / c;
                        meani2[a, b] += (imb2 - meani2[a, b]) / c;

                        //sd
                        sdb[a, b] += (bal1 - meanb[a, b]) * (bal1 - oldbal1);
                        sdb2[a, b] += (bal2 - meanb2[a, b]) * (bal2 - oldbal2);
                        sdi[a, b] += (imb1 - meani[a, b]) * (imb1 - oldimb1);
                        sdi2[a, b] += (imb2 - meani2[a, b]) * (imb2 - oldimb2);
                    }
                }
            }//end of for
            ...
            for (int i = 0; i < nodes; i ++)
            {
                for (int j =0; j < nodes; j++)
                {
                    ...
                    //calc lci
                    double lcib = meanb[i, j] - (1.96 * sdb[i, j] / 10);
                    double lcib2 = meanb2[i, j] - (1.96 * sdb2[i, j] / 10);
                    double lci = meani[i, j] - (1.96 * sdi[i, j] / 10);
                    double lci2 = meani2[i, j] - (1.96 * sdi2[i, j] / 10);

                    //calc uci
                    double ucib = meanb[i, j] + (1.96 * sdb[i, j] / 10);
                    double ucib2 = meanb2[i, j] + (1.96 * sdb2[i, j] / 10);
                    double uci = meani[i, j] + (1.96 * sdi[i, j] / 10);
                    double uci2 = meani2[i, j] + (1.96 * sdi2[i, j] / 10);


                    final[iter, 0] = meanb[i,j];
                    final[iter, 1] = sdb[i,j];
                    final[iter, 2] = lcib;
                    final[iter, 3] = ucib;
                    final[iter, 4] = meani[i,j];
                    final[iter, 5] = sdi[i,j];
                    final[iter, 6] = lci;
                    final[iter, 7] = uci;
                    final[iter, 8] = meanb2[i,j];
                    final[iter, 9] = sdb2[i,j];
                    final[iter, 10] = lcib2;
                    final[iter, 11] = ucib2;
                    final[iter, 12] = meani2[i, j] ;
                    final[iter, 13] = sdb2[i,j];
                    final[iter, 14] = lci2;
                    final[iter, 15] = uci2;
        ...
        public Matrix calcNullMatrix(bool hasDiag, double density, Matrix model)
        {
            Matrix n = new Matrix(model.Rows, model.Cols);
            Random rng = new Random();
            List<double> vals = new List<double>();           
            int fillVals = n.Rows * n.Rows;
            double curTotal = 0;
            double diff = 0;
            int randStart = 1;
            int randEnd = 3;
            ...
            if (hasDiag)
            {
                for (int i = 0; i < (fillVals - model.Rows); i++)
                {
                    int r = rng.Next(0,3);
                    vals.Add(r);
                    curTotal += r;
                } //generating random Numberlist               
            } //end of if hasDiag

            else //has diag
            {
                for (int i = 0; i < (fillVals); i++)
                {
                    int r = rng.Next(0, 3);
                    vals.Add(r);
                    curTotal += r;
            ...
            if (curTotal >= density)
            {
                ...
                while (diff != 0)
                {
                    ...
                    int r = rng.Next(randStart, randEnd);

                    if (!vals.Contains(r) && r == 2)
                    {
                        randEnd = 2;
                        r = 1;
                    }

                    else if (!vals.Contains(r) && r == 1)
                    {
                        randStart = 2;
                        r = 2;
                    }
        ...
        public void LoadMultiplex(DataGridView data, int year, string[] fnames, string curFile, string LoadFrom, bool order)
        {
            int numNodes = mTable["Data"].Cols;
            int numCells = numNodes * numNodes;
            ...
            //MultiplexProgress MP = new MultiplexProgress(order, year, fnames, LoadFrom, curFile, numCells, data, mTable);
            //MP.Show();

            /*
            //writing on grid
            t test = new t();
            test.progressBar1.Maximum = 200;
            test.Show();


            for (int i = 0; i <= 100; i++)
            {
                test.progressBar1.Value = i;
                Thread.Sleep(100);
            }
            test.Close();
            */
            ...
            if (order)
            {
                for (int i = 0; i < numNodes; i++)
                {
                    for (int j = 0; j < numNodes; j++)
                    {
                        output[dummy, 3] = plus[i, j];
                        output[dummy, 4] = neg[i, j];
                        output[dummy, 5] = bal[i, j];
                        output[dummy++, 6] = imbal[i, j];
            ...
            else
            {
                for (int i = 0; i < numNodes; i++)
                {
                    for (int j = 0; j < numNodes; j++)
                    {
                        ...
                        output[dummy, 5] = bal[i, j];
                        output[dummy, 6] = imbal[i, j];
                        output[dummy, 7] = b2[i, j];
                        output[dummy++, 8] = i2[i, j];
